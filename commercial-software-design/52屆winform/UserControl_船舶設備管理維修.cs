...
namespace _52屆winform
{
    public partial class UserControl_船舶設備管理維修 : UserControl
    {
        ...
        {
            lib.startExecuteNonQuery($"insert into 保養資料 (IMO,使用者ID,日期,狀態) select {IMO_combox.SelectedValue},'{負責人_combobox.SelectedValue}','{時間BOX.Text}','{設備狀態.Text}'");
        ...
        {
            lib.startExecuteNonQuery($"UPDATE 保養資料 set IMO={IMO_combox.SelectedValue},使用者ID='{負責人_combobox.SelectedValue}',日期='{時間BOX.Text}',狀態='{設備狀態.Text}' where 保養資料ID={dataGridView2.SelectedRows[0].Cells[0].Value} ");
        ...
        {
            lib.startExecuteNonQuery($"delete from 保養資料 where 保養資料ID={dataGridView2.SelectedRows[0].Cells[0].Value}");
